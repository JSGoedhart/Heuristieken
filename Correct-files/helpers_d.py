...
def hillclimbing_fleet(spacecrafts_fleet, spacecraft_list):
    ...
    cap_kg = list(reversed(cap[0])); cap_m3 = list(reversed(cap[1]))
    num_fleet_used = 2
    ...
    spacecrafts_fleet = merge_used_spacecrafts(spacecrafts_fleet, num_fleet_used)
...
def merge_used_spacecrafts(spacecrafts_fleet, num, spacecraft_list):
    ...
    for k in range(num):
        ...
        for i in range(len(spacecrafts_fleet[len(spacecrafts_fleet)-1-k])):
            if i != 0:
                ...
                if leftover_spacecraft:
                        spacecrafts_fleet[len(spacecrafts_fleet)-1-k][0].extend(leftover_spacecraft)
    ...
    for k in range(num):        
        if not spacecrafts_fleet[len(spacecrafts_fleet)-2-k][4]:
            spacecrafts_fleet[len(spacecrafts_fleet)-2-k][0].extend(spacecrafts_fleet[len(spacecrafts_fleet)-1-k][0])
    ...
    for i in range(len(new_leftovers)):
        print "cargo", i
        print new_leftovers[i].kg
